...
U8 usage() {
  puts("gc32: a Govno Core 32 emulator");
  puts("Syntax: gc32 [OPTIONS] <file>");
  puts("  gc32   <file>  Load the file directly to memory");
  puts("Options:");
...
U8 loadBootSector(U8* drive, U8* mem, U32 start, U32 to) {
  ...
  printf("gc32: read %d bytes from ROM\n", to-oto+1);
  return 0;
}
...
U8 main(I32 argc, I8** argv) {
  ...
  if (argc == 1) {
    old_st;
    usage();
    fprintf(stderr, "gc32: fatal error: no arguments given\n");
    return 1;
  }
  while (argp < argc) {
    // Load from the disk
  ...
  if (!driveboot) { // Load a memory dump
    ...
    if (fl == NULL) {
      fprintf(stderr, "gc32: \033[91mfatal error:\033[0m file `%s` not found\n", filename);
      old_st;
      free(gc.mem);
      free(gc.rom);
      return 1;
